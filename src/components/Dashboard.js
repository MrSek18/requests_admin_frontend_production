    ...
    const fetchUserData = async () => {
      try {
        ...
        setUserData({
          ...
          name: response.data.admin.name || "Usuario",
          email: response.data.admin.email || "No especificado",
          joinedDate:
            response.data.admin.created_at || new Date().toISOString(),
        });
      } catch (error) {
    ...
    >
      <div className="h-full w-full max-w-md space-y-6 bg-white rounded-xl shadow-lg overflow-y-auto">
        <div className="mt-14 ml-5 mr-5 flex flex-col items-center">
          <div className="w-[70%] h-[100px] flex flex-col items-center justify-center">
          ...
          <div className="text-xs w-full flex items-center justify-around mt-10 font-bold">
            ...
            <a href="/register">Registrar Admins</a>
